...
def read_urls(filename):
    ...
    reg_ex = re.compile(r'/images/animals.*?jpg', re.DOTALL)

    img_data = reg_ex.findall(s)
    img_arr = list(set(img_data))
    
    dir_names = os.listdir('.')
    domen = f'https:\\{dir_names[0][:12]}'

    img_urls = list(map(lambda x: domen + x, img_arr))
...
def download_images(img_urls, dest_dir):
    ...
    for i in range(len(img_urls)):
        filepath = os.path.join(dest_dir, f'img{i}.jpg')
        urllib.request.urlretrieve(img_urls[i], filepath)
        html_arr.append(f'<img src={filepath}>')
